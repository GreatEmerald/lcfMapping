...
# Link to data folder
linkData <- "data/" # adjust to own folder !
...
# Get XY coords and location ID
DFcoords = b1Landsat[c("x","y")]
write.csv(DFcoords, paste0(linkData, "processed/IIASAtrainingCoords.csv"), row.names=F)
DFlocations = b1Landsat["location_id"]
write.csv(DFlocations, paste0(linkData, "processed/IIASAtrainingLocationID.csv"), row.names=F)

## TEST and EXPERIMENT BELOW
# Use to make sure a small part of the processing works before running all
if (FALSE) {
...
st_write(b2FilteredSF, "data/processed/b2Filtered.gpkg")
